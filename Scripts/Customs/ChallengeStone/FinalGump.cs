...
{ 
	...
	{ 
		...
		{		
			...
			m_Item = item;
			m_Timer = new ResponceTimers( m_Item, m_OpponentMobile );
			m_Timer.Start();
		...
		public class ResponceTimers : Timer
		{
		...
		{ 
			...
			{ 
				...
				{	
					...
					foreach( PlayerMobile challenger in m_Item.ChallengeTeam )
					{
						challenger.Location = temp1; 
						temp1 = new Point3D((temp1.X + 1),temp1.Y,temp1.Z);
					}
					
					foreach( PlayerMobile opponent in m_Item.OpponentTeam )
					{
						opponent.Location = temp2;
