...
namespace Server.Scripts.Commands 
{
	public class Unload
	{
        ...
        { 
            Server.Commands.Register( "Unload", AccessLevel.Administrator, new CommandEventHandler( UnloadMain_OnCommand ) );
	    Server.Commands.Register( "UnloadMalas", AccessLevel.Administrator, new CommandEventHandler( UnloadMalas_OnCommand ) );
	    Server.Commands.Register( "UnloadIlshenar", AccessLevel.Administrator, new CommandEventHandler( UnloadIlshenar_OnCommand ) );
	    Server.Commands.Register( "UnloadTokuno", AccessLevel.Administrator, new CommandEventHandler( UnloadTokuno_OnCommand ) );
        }
        ...
        { 
			...
			string prefix = Server.Commands.CommandPrefix;
                        ...
                        if( Selections.Contains( 101 ) == true )
                        {
                            ...
                            Commands.Handle( from, String.Format( "{0}Spawngen unload 400", prefix ) );
                        }

                        if( Selections.Contains( 102 ) == true )
                        {
                            from.Say( "MALAS: Unloading North.map..." );
                            Commands.Handle( from, String.Format( "{0}Spawngen unload 403", prefix ) );
                        }

                        if( Selections.Contains( 103 ) == true )
                        {
                            from.Say( "MALAS: Unloading OrcForts.map..." );
                            Commands.Handle( from, String.Format( "{0}Spawngen unload 401", prefix ) );
                        }

                        if( Selections.Contains( 104 ) == true )
                        {
                            from.Say( "MALAS: Unloading South.map..." );
                            Commands.Handle( from, String.Format( "{0}Spawngen unload 404", prefix ) );
                        }

                        if( Selections.Contains( 105 ) == true )
                        {
                            from.Say( "MALAS: Unloading Vendors.map..." );
                            Commands.Handle( from, String.Format( "{0}Spawngen unload 402", prefix ) );
                        }
