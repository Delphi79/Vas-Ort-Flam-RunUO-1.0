...
namespace Server.Scripts.Commands
{
	public class OptionsGumps
	{
		...
		{ 
			Server.Commands.Register( "GumpSaveRegion", AccessLevel.Administrator, new CommandEventHandler( OptionsGumps1_OnCommand ) );
			Server.Commands.Register( "GumpSaveCoordinate", AccessLevel.Administrator, new CommandEventHandler( OptionsGumps2_OnCommand ) );
			Server.Commands.Register( "GumpRemoveID", AccessLevel.Administrator, new CommandEventHandler( OptionsGumps3_OnCommand ) );
			Server.Commands.Register( "GumpRemoveCoordinate", AccessLevel.Administrator, new CommandEventHandler( OptionsGumps4_OnCommand ) );
			Server.Commands.Register( "GumpRemoveRegion", AccessLevel.Administrator, new CommandEventHandler( OptionsGumps5_OnCommand ) );
		}
			...
			switch ( info.ButtonID )
			{
				...
				case 101:
				{
					...
					if( sRegion != "" )
					{
						...
						Commands.Handle( from, String.Format( "{0}Spawngen save {1}", prefix, sRegion ) );
					}
					else
					{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 101:
				{
					...
					{
						string prefix = Server.Commands.CommandPrefix;
						Commands.Handle( from, String.Format( "{0}Spawngen remove {1}", prefix, sRegion ) );
					}
					else
					{
						from.SendMessage( "You must specify a region!" );
				...
				case 101:
				{
					...
					if( sID != "" )
					{
						try
						{
							...
							string prefix = Server.Commands.CommandPrefix;
							Commands.Handle( from, String.Format( "{0}Spawngen unload {1}", prefix, UnloadID ) );
						}
						catch
						{
				...
				case 101:
				{
					...
					if( sX1 != "" && sY1 != "" && sX2 != "" && sY2 != "" )
					{
						try
						{
							...
							string prefix = Server.Commands.CommandPrefix;
							Commands.Handle( from, String.Format( "{0}Spawngen save {1} {2} {3} {4}", prefix, iX1, iY1, iX2, iY2 ) );
						}
						catch
						{
				...
				case 101:
				{
					...
					if( sX1 != "" && sY1 != "" && sX2 != "" && sY2 != "" )
					{
						try
						{
							...
							int iY2 = Convert.ToInt32( sY2 );
							string prefix = Server.Commands.CommandPrefix;
							Commands.Handle( from, String.Format( "{0}Spawngen remove {1} {2} {3} {4}", prefix, iX1, iY1, iX2, iY2 ) );
						}
						catch
						{
							from.SendMessage( "Coordinates must be numbers!" );
