...
namespace Server.Mobiles
{
	...
	public class PlayerMobile : Mobile
	{
        ...
        private bool m_ListenToGlobal = false;
		...
		private int m_Lifeforce = 0;
		private bool isinchal = false;
		private bool canbechal = true;
		private bool m_IsEvil = false;
		private bool m_IsHero = false;
		private BaseMount m_TempMount;
		private ArrayList m_duelItems = new ArrayList();

		public ArrayList DuelItems
		{
			get { return m_duelItems; }
			set { m_duelItems = value; }
		}
		...
		public int Lifeforce
		{
			...
			set { m_Lifeforce = value; }
		}

		[CommandProperty(AccessLevel.Administrator)]
		public int Spheres
		{
			get { return (m_Lifeforce / 10); }
			set { m_Lifeforce = m_Lifeforce + (value * 10); }
		}


        [CommandProperty(AccessLevel.Administrator)]
        public bool ListenToGlobal
        {
            get { return m_ListenToGlobal; }
            set { m_ListenToGlobal = value; }
        }
		...
		public override bool IsHarmfulCriminal( Mobile target )
		{
			...
			if (target is PlayerMobile && Region is DuelPit)
				return true;
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 21:
				{
					m_IsHero = reader.ReadBool();
					m_IsEvil = reader.ReadBool();
					m_Lifeforce = reader.ReadEncodedInt();

					goto case 20;
				}
				case 20:
				{
					m_Loses = reader.ReadEncodedInt();
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			{
				//if(this.FactionPlayerState.KillPoints > 0);
				//	this.FactionPlayerState.KillPoints--;
			...
			writer.Write((bool)(m_IsHero));
			writer.Write((bool)(m_IsEvil));
			writer.WriteEncodedInt((int)(m_Lifeforce));

			writer.WriteEncodedInt((int)m_Loses);
		...
		public override void OnSingleClick( Mobile from )
		{
			if (IsEvil || IsHero)
			{
				...
				if (IsEvil)
					hue = Notoriety.Hues[Notoriety.Murderer];
				else
				...
				if (ShowFameTitle && this.Fame >= 10000)
					prefix = (this.Female ? "Lady" : "Lord");

				string suffix = "";

				suffix = (IsEvil ? " (evil)" : " (hero)");
