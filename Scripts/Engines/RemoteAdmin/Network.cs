	...
	public class AdminNetwork
	{
		...
		public static void OnConsoleString( string str )
		{
			...
			for (int i=0;i<m_Auth.Count;i++)
				if ( ((NetState)m_Auth[i]).Account != null && ((Account)((NetState)m_Auth[i]).Account).AccessLevel >= AccessLevel.Administrator )
		...
		public static void OnReceive( NetState state, PacketReader pvSrc )
		{
			...
			{
				Console.WriteLine( "REMOTE: Unauthorized packet from {0}, disconnecting", state );
				Disconnect( state );
			}
			else
			{
		...
		public static void Authenticate( NetState state, PacketReader pvSrc )
		{
			...
			if ( a == null )
			{
				...
				Console.WriteLine( "REMOTE: Invalid username '{0}' from {1}", user, state );
				DelayedDisconnect( state );
			}
			else if ( !a.HasAccess( state ) )
			{
				state.Send( new Login( LoginResponse.BadIP ) );
				Console.WriteLine( "REMOTE: Access to '{0}' from {1} denied.", user, state );
				DelayedDisconnect( state );
			}
			else if ( !a.CheckPassword( pw ) )
			{
				state.Send( new Login( LoginResponse.BadPass ) );
				Console.WriteLine( "REMOTE: Invalid password '{0}' for user '{1}' from {2}", pw, user, state );
				DelayedDisconnect( state );
			}
			else if ( a.AccessLevel < AccessLevel.Counselor || a.Banned )
			{
				Console.WriteLine( "REMOTE: Account '{0}' does not have access. Connection Denied.", user );
				state.Send( new Login( LoginResponse.NoAccess ) ); 
				DelayedDisconnect( state );
			}
			else
			{
				Console.WriteLine( "REMOTE: Access granted to '{0}' from {1}", user, state );
				state.Account = a;
				a.LogAccess( state );
				a.LastLogin = DateTime.Now;

				state.Send( new Login( LoginResponse.OK ) );
				state.Send( new StateInfo( state ) );
				state.Send( Compress( new AllPages() ) );
		...
		public static void Distribute( Packet p )
		{
			Distribute( p, true );
		}

		public static void Distribute( Packet p, bool compress )
		{
			if ( compress )
				p = Compress( p );

			foreach ( NetState state in m_Auth )
			{
				state.Send( p );
			}
		}
