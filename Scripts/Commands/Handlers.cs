...
namespace Server.Scripts.Commands
{
	public class CommandHandlers
	{
		...
		public static void CountObjects_OnCommand( CommandEventArgs e )
		{
			using ( StreamWriter op = new StreamWriter( "objects.log" ) )
			{
				...
				foreach (Item i in totalItems)
				{
					...
					op.WriteLine("Name: {0}\t||	Item Type: {1}\t|| Location: {2}", i.Name, i.GetType(), i.GetWorldLocation());
				}
		...
		{
			StaffChat( e.Mobile, e.ArgString );
		}

		public static void StaffChat(Mobile from, string message)
		{
			string text = String.Format( "[{0}] {1}", from.Name, message );
			BroadcastMessage( AccessLevel.Counselor, from.SpeechHue, text );
			Server.Admin.AdminNetwork.Distribute( new Server.Admin.StaffChat( text ) );
		}
